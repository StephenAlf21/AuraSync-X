// --- GLOBAL STATE & VARIABLES ---
    ...
    function removeTrack(index) {
      ...
      if (removedCurrent) {
        ...
        if (playlist[index] && playlist[index].type === 'youtube') {
          hideYouTube();
        }
        videoPlayer.pause();
    ...
    function showYouTube(id){
        ...
        // Disable native controls while YouTube is active
        setControlsEnabled(false);
        currentTimeEl.textContent = '—';
        durationEl.textContent = 'YouTube';
    ...
    function setControlsEnabled(enabled){
        ...
        progressWrapper.style.pointerEvents = enabled ? 'auto' : 'none';
    }
    ...
    function togglePlayPause() {
      ...
      if (track && track.type === 'youtube') {
        showAlert('Use the YouTube player controls in the video to play/pause.', 'YouTube Playback');
        return;
      }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      addUrlBtn.addEventListener('click', () => {
        ...
        // YouTube support (no API key required)
        if (isYouTubeUrl(url)) {
          const vid = extractYouTubeId(url);
          if (!vid) {
            showAlert('Could not parse YouTube video ID from the URL.', 'Invalid YouTube URL');
            return;
          }
          playlist.push({ file: null, url: url, name: `YouTube: ${vid}`, artist: 'YouTube', type: 'youtube', videoId: vid });
        } else if (/^https?:\/\/.+\.(mp4|webm|ogv)(\?.*)?$/i.test(url)) {
          const filename = url.split('/').pop().split('?')[0];
          playlist.push({ file: null, url: url, name: decodeURIComponent(filename), artist: 'Remote URL' });
        } else {
          showAlert('Please enter a direct link to a video file (mp4, webm, ogv) or a YouTube URL.', 'Invalid URL');
          return;
        }
